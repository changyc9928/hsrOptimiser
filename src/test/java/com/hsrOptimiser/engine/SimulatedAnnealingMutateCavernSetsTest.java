...
import com.hsrOptimiser.engine.strategies.CavernSetMutationStrategy;
...
class SimulatedAnnealingMutateCavernSetsTest {
    ...
    private CavernSetMutationStrategy strategy;

    @BeforeEach
    void setUp() {
        lenient().when(random.nextInt(anyInt())).thenReturn(0);
        strategy = new CavernSetMutationStrategy();
    }

    private Relic relic(String setId, Slot slot, String location) {
    ...
    private void mutateCavernSets(ScannedData data, String characterId,
        List<Slot> slots, List<Relic> currentEquipped,
        Set<String> allowed, Set<String> disallowed, Random random) {
        MutationContext context = new MutationContext(
            characterId, 0, allowed, disallowed, random);
    ...
    void shouldClearExistingCavernRelicsBeforeEquippingNewOnes() {
        ...
        long equippedCount = relics.stream()
            .filter(r -> CHARACTER_ID.equals(r.getLocation()))
            .count();
    ...
    void shouldReturnWhenOneSlotHasNoAvailableRelics() {
        ...
        assertThat(data.getRelics().stream()
            .noneMatch(r -> CHARACTER_ID.equals(r.getLocation()))).isTrue();
    }
    ...
    void shouldAllowRelicsOwnedByAllowedCharacters() {
        ...
        mutateCavernSets(
            data, CHARACTER_ID,
            List.of(Slot.Head, Slot.Hands, Slot.Body, Slot.Feet),
            List.of(), Set.of("OTHER"), Set.of(), random);
    ...
    void shouldRejectRelicsOwnedByDisallowedCharacters() {
        ...
        mutateCavernSets(
            data, CHARACTER_ID,
            List.of(Slot.Head, Slot.Hands, Slot.Body, Slot.Feet),
            List.of(), Set.of("OTHER"), Set.of("OTHER"), random);
    ...
    void shouldAllowFourPieceSetWhenAllSlotsMatchSameSet() {
        ...
        long equipped = data.getRelics().stream()
            .filter(r -> CHARACTER_ID.equals(r.getLocation()))
            .count();
